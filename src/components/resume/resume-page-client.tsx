...
import { resumeListQuery } from "./resume-queries";
import type { ResumeSort } from "./resume-sort";
import ResumeSortDropdown from "./resume-sort-dropdown";

export function ResumePageClient({ sort }: { sort: ResumeSort }) {
  const trpc = useTRPC();
  const { data: resumes } = useSuspenseQuery(resumeListQuery(trpc, sort));
